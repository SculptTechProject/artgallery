...
// Pamiętaj o poprawionym types.ts!
...
export default function ExhibitionsAdminPage() {
  ...
  const handleDelete = async (e: React.MouseEvent, id: number) => {
    e.stopPropagation();
    if (confirm("Czy na pewno chcesz usunąć tę wystawę?")) {
  ...
  const columns = [
      ...
      accessor: (ex: any) =>
        ex.imageUrl ? (
          <img
            src={ex.imageUrl}
            alt={ex.name}
            className="w-8 h-4 object-cover rounded border border-[#4E5155]"
          />
        ) : (
      ...
      accessor: (ex: any) => (
        <div className="flex items-center gap-1.5 text-[10px]">
          ...
          <span className="whitespace-nowrap">
            {new Date(ex.startDate).toLocaleDateString("pl-PL")} -{" "}
            {new Date(ex.endDate).toLocaleDateString("pl-PL")}
          </span>
        </div>
      ),
    },
    {
      header: "ZAJĘTOŚĆ",
      accessor: (ex: any) => {
        // Obliczamy procent zajętości dla paska
        const percent = Math.min(
          100,
          Math.round(((ex.soldCount || 0) / (ex.capacity || 100)) * 100)
        );
        const isFull = (ex.soldCount || 0) >= (ex.capacity || 100);

        return (
          <div className="flex flex-col gap-1 w-full max-w-[120px]">
            <div className="flex items-center justify-between text-[10px] font-mono">
              <span className={isFull ? "text-red-400" : "text-white"}>
                {ex.soldCount || 0} / {ex.capacity || 100}
              </span>
              <span className="text-[#4E5155]">{percent}%</span>
            </div>
            {/* Pasek postępu */}
            <div className="h-1 w-full bg-[#2B2D30] rounded-full overflow-hidden">
              <div
                className={`h-full ${
                  isFull ? "bg-red-500" : "bg-[#3574F0]"
  ...
  return (
    <div className="flex flex-col h-full bg-[#1E1F22]">
      ...
      <div className="h-10 bg-[#2B2D30] border-b border-[#4E5155] flex items-center px-4 gap-4 flex-shrink-0">
        ...
        {error && (
          <div className="text-red-400 text-[10px] ml-auto uppercase font-bold">
            {error}
          </div>
        )}
      </div>
